...
class Device():
	...
	def __init__(self):
		...
		self.Helpers.logger.info("iotJumpWay Device Device Initiated.")
	...
	def on_subscribe(self, client, obj, mid, granted_qos):
		...
		self.Helpers.logger.info("iotJumpWay Device Subscription: "+str(mid))

	def on_message(self, client, obj, msg):

		self.Helpers.logger.info("iotJumpWay Device Message Received")
		splitTopic=msg.topic.split("/")

		if splitTopic[1]=='Devices':
			if splitTopic[4]=='Commands':
				if self.commandsCallback == None:
					self.Helpers.logger.info("** Device Commands Callback Required (commandsCallback)")
				else:
